...
const STR = {
  ...
  bookmarkFail: {
    en: 'Could not fetch bookmark metadata for %s — leaving it as a plain link.',
    es: 'No pude obtener los metadatos del bookmark %s — lo dejo como enlace normal.',
    fr: 'Impossible de récupérer les métadonnées du signet %s — laissé en lien simple.',
    de: 'Bookmark-Metadaten für %s nicht abrufbar — bleibt als einfacher Link.',
    pt: 'Não consegui obter os metadados do bookmark %s — fica como link normal.',
    it: 'Impossibile recuperare i metadati del bookmark %s — lasciato come link semplice.',
    nl: 'Kon bookmark-metadata voor %s niet ophalen — blijft een gewone link.'
  },
  overwriteBlocked: {
    en: 'A post with slug "%s" already exists (title: "%s", status: %s). Not overwriting.',
    es: 'Ya existe una entrada con slug "%s" (título: "%s", status: %s). No la sobrescribo.',
    fr: 'Un article avec le slug "%s" existe déjà (titre : "%s", statut : %s). Pas d\'écrasement.',
    de: 'Es existiert bereits ein Beitrag mit Slug "%s" (Titel: "%s", Status: %s). Wird nicht überschrieben.',
    pt: 'Já existe uma publicação com slug "%s" (título: "%s", status: %s). Não vou sobrescrever.',
    it: 'Esiste già un articolo con slug "%s" (titolo: "%s", stato: %s). Non lo sovrascrivo.',
    nl: 'Er bestaat al een bericht met slug "%s" (titel: "%s", status: %s). Wordt niet overschreven.'
  },
  overwriteHint: {
    en: 'Re-run with --force to overwrite it on purpose.',
    es: 'Vuelve a ejecutar con --force para sobrescribirla a propósito.',
    fr: 'Relancez avec --force pour l\'écraser volontairement.',
    de: 'Führe es mit --force erneut aus, um ihn absichtlich zu überschreiben.',
    pt: 'Executa de novo com --force para sobrescrever de propósito.',
    it: 'Riesegui con --force per sovrascriverlo di proposito.',
    nl: 'Voer opnieuw uit met --force om het bewust te overschrijven.'
  },
  verifying: {
