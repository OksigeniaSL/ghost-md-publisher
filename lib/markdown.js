 ...
 *   - `::video <url>` en línea propia → <figure class="kg-card kg-embed-card">
 *     con iframe de YouTube o Vimeo según corresponda.
...
function buildVideoFigure(iframeHtml, caption) {
  if (!caption) {
...
<!--kg-card-end: embed-->`;
  ...
  const captionHtml = `<figcaption><p><span style="white-space: pre-wrap;">${escapeHtml(caption)}</span></p></figcaption>`;
  return `<!--kg-card-begin: embed-->
...
function transformVideoShortcodes(md) {
  return md.replace(
    ...
    (match, url, caption) => {
      const yt = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([\w-]{6,20})/);
      const vm = url.match(/vimeo\.com\/(?:video\/)?(\d+)/);
      if (yt) {
        const id = yt[1];
        const iframe = `<iframe width="640" height="360" src="https://www.youtube.com/embed/${id}?feature=oembed" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>`;
        return buildVideoFigure(iframe, caption ? caption.trim() : null);
      }
      if (vm) {
        const id = vm[1];
        const iframe = `<iframe width="640" height="360" src="https://player.vimeo.com/video/${id}" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>`;
        return buildVideoFigure(iframe, caption ? caption.trim() : null);
      }
