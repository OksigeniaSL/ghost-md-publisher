 ...
 */
async function upsertPost(client, frontmatter, html, { dryRun = false } = {}) {
  const payload = {
  ...
  try {
    ...
    if (err.code !== 'NOT_FOUND' && !/not found/i.test(err.message || '')) throw err;
  }
  ...
  if (existing) {
    ...
    if (existing.status && existing.status !== payload.status) {
      console.log(`  ! Post ya existe con status="${existing.status}"; preservo ese estado (front-matter pedía "${payload.status}")`);
